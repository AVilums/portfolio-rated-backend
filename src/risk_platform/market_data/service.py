...
from risk_platform.market_data.connectors import EtfConnector
...
def ingest(db: Session, connector: EtfConnector) -> list[UUID]:
    """Atomic batch; caller owns commit. Exact retries reuse the stored snapshot."""
    snapshots = [EtfSnapshot.model_validate(s.model_dump()) for s in connector.fetch()]
    ids: list[UUID] = []
